...
def stellarLocusFits(cat, magColName="PsMag"):
    """Calculates stellar locus fit paramaters
    ...
    Notes
    -----
    This will probably be replaced by a dedicated task to do the fitting in
    the future. It is mainly here for testing purposes
    """

    # TODO: this will probably be replaced by a dedicated task to do the
    # fitting.
...
def addUseForQAFlag(cat):
    ...
    # TODO: This needs to be checked and probably changed
    ...
    for band in ["g", "r", "i", "z", "y"]:
        usePerBand = ((cat[band + "Centroid_flag_notAtMaximum"] == 0) & (cat[band + "Shape_flag"] == 0)
                      ...
                      & (cat[band + "Deblend_nChild"] == 0))
        use = use & usePerBand
    cat["useForQAFlag"] = use
    return cat


def addSNColumn(cat, fluxColName="PsFlux"):
    ...
    for band in ["g", "r", "i", "z", "y"]:
        SN = cat[band + fluxColName].values / cat[band + fluxColName + "Err"].values
        cat[band + "Sn" + fluxColName] = SN
...
def addUseForStatsColumn(cat):
    ...
    # TODO: Is this needed? Where do the thresholds come from?
...
def addDeconvMoments(cat):
    ...
    cat : `pandas.core.frame.DataFrame`
    """
    # TODO: Is this now these should be calculated?
