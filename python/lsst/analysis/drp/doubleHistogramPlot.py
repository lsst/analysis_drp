...
class DoubleHistogramTaskConnections(pipeBase.PipelineTaskConnections, dimensions=("tract", "skymap"),
    ...
    doubleHistogramPlot = pipeBase.connectionTypes.Output(doc="A double histogram plot.",
                                                          storageClass="Plot",
                                                          name="doubleHistogram_{plotName}",
...
class DoubleHistogramTaskConfig(pipeBase.PipelineTaskConfig,
                                pipelineConnections=DoubleHistogramTaskConnections):
    ...
    objectsToPlot = pexConfig.Field(
        doc=("Which types of objects to include on the plot, should"
             "be one of 'stars', 'galaxies' or 'all'."),
        dtype=str,
        default="stars",
    )


class DoubleHistogramTask(pipeBase.PipelineTask):

    ConfigClass = DoubleHistogramTaskConfig
    _DefaultName = "doubleHistogramTask"
    ...
    def run(self, catPlot, dataId, runName, skymap):
        ...
        fig = self.doubleHistogram(catPlot, xLabel, yLabel, title, plotInfo, sumStats)

        return pipeBase.Struct(doubleHistogramPlot=fig)

    def doubleHistogram(self, catPlot, xLabel, yLabel, title, plotInfo, sumStats,
        ...
        Notes
        -----
        Uses the config options `xColName` and `yColName` to plot a scatter
        ...
        of the resultant plot. The code uses the columns labelled `useForStats`
        and `useForQAFlag` to determine which points to plot. More details on
        these can be found in the addQAColunms task.
        """
        ...
        colsToPlot = ["base_CircularApertureFlux_9_0_instFlux",
        ...
        for col in colsToPlot:
            ax1.hist(catPlot[col])

        # second histogram
        ax2 = fig.add_subplot(gs[-3:,:])

        for col in colsToPlot:
            ax2.hist(catPlot[col])
        ...
        axCorner = plt.gcf().add_subplot(gs[:2, -2:])
