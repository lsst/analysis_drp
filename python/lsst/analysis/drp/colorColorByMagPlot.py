...
class ColorColorByMagPlotTask(pipeBase.PipelineTask):
    ...
    def run(self, catPlot, dataId, runName, skymap):
        """Makes a four panel color-color plot sub divided by magnitude
        """
    ...
    def colorColorByMagPlot(self, catPlot, xLabel, yLabel, plotInfo, sumStats, yLims=False, xLims=False):
        """Make a four panel color color plot sub divided by magnitude
        ...
        Notes
        -----
        Makes a four panel color-color plot of `self.config.xColName` against
        `self.config.yColName`, each panel shows a specific magnitude range.
        The points on the plot are color coded by i band CModel magnitude. The
        axis labels are given by `self.config.xLabel` and `self.config.yLabel`.
        The column given in `self.config.sourceTypeColName` is used for star
        galaxy separation.
        """
        ...
        # TODO: finalise these once we finalise which points go on the
        # plots.
        starPercsX = np.nanpercentile(xsStars, [2.0, 98.5])
        starPercsY = np.nanpercentile(ysStars, [1.0, 99.5])
